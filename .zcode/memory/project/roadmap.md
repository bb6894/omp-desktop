...
| ------ | ------------------------------------------------- | ----------- | ----- |
| MVP    | Core `npm install -g github:cong91/zcode-starterkit` command with basic template | Complete    | [#]   |
...
**Goal:** Users can run `npm install -g github:cong91/zcode-starterkit` to generate a working ZCode project template.

**Success Criteria:**

- [x] `npm install -g github:cong91/zcode-starterkit` command creates project structure
...
**Goal:** Add essential ZCode commands for common workflows.
...
| -   | Install integration harden (`--skip-*` non-destructive + healthcheck) | feature | open | Polish |

### Install integration harden (queued design)

See `gotchas.md` → *Install: `--skip-codebase-memory` is destructive today* and *Future harden*.

Goal: stop accidental `--skip-codebase-memory` / `--skip-webclaw` reinstalls from unregistering healthy MCP entries; add post-install health matrix (binary / MCP entry / state / index); keep observation-FTS5 diagnosis separate from Codebase-Memory graph availability.
